...
namespace docwire
{
	...
	static void appendToString(void* stream, const char* text, int size) {
		...
		if (stream) {
			...
			fprintf(stderr, "appnd2str(%i): %s\n", size, text);
#endif
			auto val = (std::ostringstream*)stream;
			if (!(strlen(text) > size)) {
				std::string txt(text);
				*val << txt.substr(0, size).c_str();
			}
		}
	}
	...
	static std::string getInfoString(Object* infoDict, const char* infoKey,
		...
		if (value) {
			res.append(value->getCString());
			delete value;
		}
		return res;
	}

	typedef struct PDFDoc_deleter {
		void operator()(void* p) {
			if (p != NULL) delete p;
		}
	} PDFDoc_deleter_t;
	typedef std::unique_ptr<PDFDoc, PDFDoc_deleter_t> pdf_doc_ptr;

	typedef struct GlobalParams_deleter {
		void operator()(void* p) {
			if (p != NULL) delete p;
		}
	} GlobalParams_deleter_t;

	typedef std::unique_ptr<GlobalParams, GlobalParams_deleter_t> global_params_ptr;

	typedef struct UnicodeMap_deleter {
		void operator()(void* p) {
			if (p != NULL) {
				UnicodeMap* ptr = (UnicodeMap*)p;
				ptr->decRefCnt();
			}
		}
	} UnicodeMap_deleter_t;

	typedef std::shared_ptr<UnicodeMap> unicode_map_ptr;
	...
	struct pimpl_impl<PDFParser> : with_pimpl_owner<PDFParser>
	{
		...
		// unicode_map_ptr unicode_map;
	};

	void PDFParser::loadDocument(const data_source& data)
	{
		docwire_log_func();
		...
		if (data.path().has_value()) {
			try
			{
				...
				auto pathString = new GString(data.path().value().string().c_str());
				auto pdfdoc = new PDFDoc(pathString, NULL, NULL, NULL);
				impl().doc = pdf_doc_ptr(pdfdoc);
				// check for copy permission
				throw_if(!impl().doc->okToCopy(), "Copying of text from this document is not allowed.");
				delete pathString;
			}
			catch (GMemException& e)
			{
				std::throw_with_nested(make_error("Out of memory"));
			}
		}
	}
	...
	PDFParser::PDFParser() {
		...
		globalParams = new GlobalParams(NULL);
		globalParams->setTextEncoding("Latin1");
		/* std::string textEOL = "unix";
		if (!globalParams->setTextEOL(textEOL.data())) {
#if defined(_DEBUG) || defined(DEBUG)
		...
		const char* eol = "unix";
		globalParams->setTextEOL(const_cast<char*>(eol));
		// get mapping to output encoding
		// auto umap = globalParams->getUnicodeMap(globalParams->getTextEncodingName());
		// throw_if(umap == NULL, "Couldn't get text encoding");
		// impl().unicode_map = unicode_map_ptr(umap);
		// impl().params = global_params_ptr(globalParams);
	}

	std::mutex xpdf_mutex;


	PDFParser::~PDFParser() {
		destroy_impl();
		// if (globalParams != NULL) delete globalParams;
	}

	void PDFParser::parseMetadata(attributes::Metadata& metadata)
	{
		...
		Object info;
		GString* m_metadata = NULL;
		ZxDoc* xmp = NULL;
		UnicodeMap* uMap = globalParams->getUnicodeMap(globalParams->getTextEncodingName());
		impl().doc->getDocInfo(&info);
		if ((m_metadata = impl().doc->readMetadata())) {
			xmp = ZxDoc::loadMem(m_metadata->getCString(), m_metadata->getLength());
		}

		if (m_metadata && xmp)
		{
			auto value = std::string(getInfoString(&info, "Title", xmp, "dc:title", NULL, gFalse, uMap));
			if (!value.empty()) {
				metadata.title = value;
			}
			value = std::string(getInfoString(&info, "Subject", xmp, "dc:description", NULL, gFalse, uMap));
			if (!value.empty()) {
				metadata.subject = value;
			}
			value = std::string(getInfoString(&info, "Keywords", xmp, "pdf:Keywords", NULL, gFalse, uMap));
			if (!value.empty()) {
				metadata.keywords = value;
			}
			value = std::string(getInfoString(&info, "Author", xmp, "dc:creator", NULL, gFalse, uMap));
			if (!value.empty()) {
				metadata.author = value;
			}

			value = std::string(getInfoString(&info, "CreationDate", xmp, "xap:CreateDate", "xmp:CreateDate", false, uMap));
			if (!value.empty()) {
				struct tm tm_struct;
				strptime(value.c_str(), "%c", &tm_struct);
				metadata.creation_date = tm_struct;
			}

			value = std::string(getInfoString(&info, "ModDate", xmp, "xap:ModifyDate", "xmp:ModifyDate", false, uMap));
			if (!value.empty()) {
				struct tm tm_struct;
				strptime(value.c_str(), "%c", &tm_struct);
				metadata.last_modification_date = tm_struct;
			}
			metadata.page_count = impl().doc->getNumPages();
		}
		if (xmp) {
			delete xmp;
		}
	...
	attributes::Metadata PDFParser::metaData()
	{
	...
	void PDFParser::parseText()
	{
		TextOutputControl textOutControl;
		textOutControl.clipText = gFalse;
		textOutControl.mode = TextOutputMode::textOutRawOrder;
		textOutControl.fixedPitch = 0;
		textOutControl.discardDiagonalText = gTrue;
		textOutControl.discardInvisibleText = gTrue;
		// textOutControl.splitRotatedWords = gFalse;
		textOutControl.overlapHandling = TextOutputOverlapHandling::textOutIgnoreOverlaps;
		textOutControl.insertBOM = gFalse;
		textOutControl.marginLeft = 0;
		textOutControl.marginRight = 0;
		textOutControl.marginTop = 0;
		textOutControl.marginBottom = 0;

		docwire_log_func();
		int page_count = impl().doc->getNumPages();
		...
		try
		{
			TextOutputDev textOut(appendToString, (void*)&oss, &textOutControl);
			if (textOut.isOk()) {
				impl().doc->displayPages(&textOut, 1, page_count, 72, 72, 0, gFalse, gTrue, gFalse, NULL, NULL);
			}
			std::string output = oss.str();


			// Function to split a string into tokens based on a delimiter
			auto clean_text = [](std::string input) {
				std::sregex_token_iterator end;

				// Remove page numbers
				std::regex page_number("(\\s*\\n)([0-9]+)(\\s*\\n)", std::regex_constants::ECMAScript );
				std::sregex_token_iterator excl_pg(input.begin(), input.end(), page_number, -1);
				std::vector<std::string> cleaning_stage_1(excl_pg, end);
				std::string post_stage_1 = std::reduce(cleaning_stage_1.begin(), cleaning_stage_1.end(), std::string());

				// Remove control characters - except tabs and newlines
				std::regex non_priniting("[\\x00-\\x08\\x0B-\\x1F\\x7F]", std::regex_constants::ECMAScript);
				std::sregex_token_iterator it(post_stage_1.begin(), post_stage_1.end(), non_priniting, -1);
				std::vector<std::string> cleaning_stage_2(it, end);
				std::string post_stage_2 = std::reduce(cleaning_stage_2.begin(), cleaning_stage_2.end(), std::string());

				return post_stage_2;

			};

			std::string result = clean_text(oss.str());

			sendTag(tag::Text{ result });
		}
		catch (const std::exception& e)
		{
			std::throw_with_nested(make_error(e.what()));
		}
	}

	void PDFParser::parse(const data_source& data)
	{
		docwire_log(debug) << "Using PDF parser.";
		{
			std::lock_guard<std::mutex> xpdf_mutex_lock(xpdf_mutex);
			renew_impl();
		}
		loadDocument(data);
