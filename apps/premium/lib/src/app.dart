...
class _MyHomePageState extends State<MyHomePage> with WidgetsBindingObserver {
  ...
  final ChatWidgetController chatWidgetController = June.getState(
  ...
  void initState() {
    ...
    {
      StatusMonitor.updates.listen(
        (event) async {
          if (event.status == StatusEventCode.NEW) {
            UnnuKnow.instance.listener.add(event);
          } else if (event.status == StatusEventCode.PROCESSED) {
            settingsController.update(event.attachment);
  ...
  Future<AppExitResponse> didRequestAppExit() async {
    ...
    await settingsController.persist();
    RagLite.instance.destroy();
  ...
  Widget build(BuildContext context) {
    ...
    final _ = June.getState(OfflineTtsController.new)
                                    ...
                                    children: [
                                      ...
                                      IconButton.outlined(
                                        ...
                                        onPressed:
                                            () => layout.onPrimaryNavigation(
                                              context,
                                              AppPrimaryNavigation.About.value,
                                            ),
                                        icon: const Icon(Icons.info),
                                      ),
                                    ],
