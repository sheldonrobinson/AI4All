...
namespace docwire
{
	...
	typedef std::unique_ptr<PDFDoc> pdf_doc_ptr;
	typedef std::unique_ptr<GlobalParams> global_params_ptr;
	...
	void PDFParser::loadDocument(const data_source& data)
	{
		...
		if (data.path().has_value()) {
			try
			{
				...
				auto path = new GString(data.path().value().string().c_str());
				auto pdfdoc = new PDFDoc(path, NULL, NULL, NULL);
	...
	PDFParser::PDFParser() {
		...
		impl().textOutControl.clipText = gFalse;
		impl().textOutControl.mode = TextOutputMode::textOutReadingOrder;
		impl().textOutControl.discardDiagonalText = gTrue;
		impl().textOutControl.discardInvisibleText = gTrue;
		impl().textOutControl.splitRotatedWords = gTrue;
		impl().textOutControl.overlapHandling = textOutAppendOverlaps;
		GlobalParams::defaultTextEncoding = "UTF-8";
		auto params = new GlobalParams(NULL);
		impl().params = global_params_ptr(params);
		impl().params->setTextEncoding(GlobalParams::defaultTextEncoding);
		char textEOL[5] = "unix";
		impl().params->setTextEOL(textEOL);
		impl().params->setupBaseFonts(NULL);

		impl().params->setTextPageBreaks(gFalse);
		impl().params->setPrintStatusInfo(gFalse);
		impl().params->setErrQuiet(gTrue);
		// get mapping to output encoding
		auto umap = globalParams->getTextEncoding();
		throw_if(umap == nullptr, "Couldn't get text encoding");
	...
	void PDFParser::parseMetadata(attributes::Metadata& metadata)
	{
		...
		if (m_metadata)
		{
			auto value = std::string(getInfoString(&info, "Title", xmp, "dc:title", NULL, gFalse, impl().unicode_map.get()));
			if (!value.empty()) {
				metadata.title = value;
			}
			value = std::string(getInfoString(&info, "Subject", xmp, "dc:description", NULL, gFalse, impl().unicode_map.get()));
			if (!value.empty()) {
				metadata.subject = value;
			}
			value = std::string(getInfoString(&info, "Keywords", xmp, "pdf:Keywords", NULL, gFalse, impl().unicode_map.get()));
			if (!value.empty()) {
				metadata.keywords = value;
			}
			value = std::string(getInfoString(&info, "Author", xmp, "dc:creator", NULL, gFalse, impl().unicode_map.get()));
			if (!value.empty()) {
				metadata.author = value;
			}

			value = std::string(getInfoString(&info, "CreationDate", xmp, "xap:CreateDate", "xmp:CreateDate", false, impl().unicode_map.get()));
			if (!value.empty()) {
				struct tm tm_struct;
			...
			value = std::string(getInfoString(&info, "ModDate", xmp, "xap:ModifyDate", "xmp:ModifyDate", false, impl().unicode_map.get()));
			if (!value.empty()) {
				struct tm tm_struct;
				strptime(value.c_str(), "%c", &tm_struct);
	...
	void PDFParser::parseText()
	{
		...
		for (size_t page_num = 0; page_num < page_count; page_num++)
		{
			...
			auto response = sendTag(tag::Page{});
			if (response.skip)
			{
				continue;
			}
			...
			try
			{
				...
				TextOutputDev textOut(appendToString, (void*)  &oss, &impl().textOutControl);
				if (textOut.isOk()) {
					impl().doc->displayPages(&textOut, page_num, 72, 72, 0, gFalse, gTrue, gFalse, NULL, NULL);
				}
				auto response = sendTag(tag::Text{ single_page_text });
				if (response.cancel)
				{
					break;
				}
				auto response2 = sendTag(tag::ClosePage{});
				if (response2.cancel)
				{
					break;
				}
