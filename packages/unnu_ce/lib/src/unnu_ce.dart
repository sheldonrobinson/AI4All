...
class UnnuCE {
  ...
  static void agent(String script_path, CNERConfig cnerconfig) {
    _init();

    final token_path = cnerconfig.tokenizer.toNativeUtf8();
    final _cfg = ffi.calloc<UnnuTokenizerConfig>();
    _cfg.ref.typeAsInt = cnerconfig.type.index;
    _cfg.ref.tokenizer_path = token_path.cast<Char>();
    _cfg.ref.length = token_path.length;
    final model = cnerconfig.path.toNativeUtf8();
    unnu_cner_init(model.cast<Char>(),_cfg.ref);
    ffi.calloc.free(token_path);
    ffi.calloc.free(_cfg);
    ffi.calloc.free(model);
...
/*
  void research(int csn, String sentence){
